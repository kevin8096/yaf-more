...
class ZookeeperClient
{
    ...
    public function getNodeJson($obj,$pNode){
        $cartNode = $obj->getChildren($pNode);

    /*    $rate = '';
        $nodeKeys = array_keys($cartNode);
        if(count($nodeKeys)==1){
            $nodeJson =  $obj->get($pNode.$cartNode[0]);
            return $nodeArr =  json_decode($nodeJson,true);
        }else{
            $probability = round(100/count($nodeKeys));
        ...
        $rate = rtrim($rate,':');
        $pollingRes = Utility::GetRandom($nodeKeys,$rate);

        if(isset($cartNode[$pollingRes])){
            $nodeJson =  $obj->get($pNode.$cartNode[$pollingRes]);
            //Yii::log('zk node:'.date('Y-m-d H:i:s').' : '.json_encode($nodeJson), 'error', 'performance');
            return $nodeArr =  json_decode($nodeJson,true);
    ...
    public static function init(){
        $zookeeperAddressIp = '';
        $zookeeperAddressIpArr = \Yaf\Application::app()->getConfig()->application->zookeeperAddressIp;
        if(count($zookeeperAddressIpArr)>1){
            $zookeeperAddressIp = implode(',',$zookeeperAddressIpArr);
        }else{
            $zookeeperAddressIp = $zookeeperAddressIpArr[0];
        }
        return $zk = new ZookeeperClient($zookeeperAddressIp);
    }
